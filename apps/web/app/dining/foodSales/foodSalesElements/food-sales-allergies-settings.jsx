...
import {Button, FieldError, Label, ListBox, ListBoxItem, ListBoxSection, Popover, Select, SelectValue, Text, Header, Collection} from 'react-aria-components';
...
export function FoodSalesAllergiesSettings() {
  ...
  const specificSubstanceTable = [
    {name: '', children: [
      {id: 0, name: `${specificSubstanceList.find(item => item.id == 0)}`}
    ]},
  ...
  for (let i = 0; i < specificSubstanceTable.length; i++) {
    if (specificSubstanceTable[i].name == '') {
      specificSubstanceTable[i].children.push({id: 0, name: `${specificSubstanceList.find(item => item.id == 0).name}`})
    }
    else if (specificSubstanceTable[i].name == '特定原材料8品目') {
      for (let j = 1; j < 8; j++) {
        specificSubstanceTable[i].children.push({id: j, name: `${specificSubstanceList.find(item => item.id == j).name}`})
      }
    }
    else if (specificSubstanceTable[i].name == '特定原材料+20品目') {
      for (let j = 9; j < 28; j++) {
        specificSubstanceTable[i].children.push({id: j, name: `${specificSubstanceList.find(item => item.id == j).name}`})
      }
    }
  ...
  if ([...selected].length > 1 && [...selected].slice(-1)[0] == 0)// 複数選択されていて、かつ、「選択しない」が選択されたとき、それ以外の選択を外す
