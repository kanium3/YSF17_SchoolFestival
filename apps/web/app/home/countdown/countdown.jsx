...
export default function Countdown() {
  ...
  const change_text = () => {
    ...
    const state
            = now < openingTime ? 'unopen' : (now < closingTime ? 'opened' : 'closed')
  ...
  if (timeInfo.state == 'unopen') {
    content = (
      <div id="countdown_display">
        <span>{timeInfo.times[0]}</span>
        日
        <span>{timeInfo.times[1]}</span>
        時間
        <span>{timeInfo.times[2]}</span>
        分
        <span>{timeInfo.times[3]}</span>
        秒
      </div>
    )
  }
